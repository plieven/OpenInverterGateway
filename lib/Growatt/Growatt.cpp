...
#error "Unsupported Growatt Modbus version"
  ...
  for (int i = 0; i < _Protocol.InputRegisterCount; i++) {
    int regCount = _Protocol.InputRegisters[i].size == SIZE_16BIT ||
                   _Protocol.InputRegisters[i].size == SIZE_16BIT_S ? 1 : 2;
    if (_Protocol.InputRegisters[i].address <= lastIReg) {
      ...
      Log.print(F(" is not defined in ascending order to previous InputRegister address "));
...
void inline Growatt::metricsAddValue(const String& name, const double& value, const float& resolution,
                              String& metrics, const String& labels) {
  char nameSnakeCase[name.length() + 10];
  camelCaseToSnakeCase(name, nameSnakeCase);
    const String svalue = (resolution == 1) ? String((int32_t)value) : String(value);
...
void Growatt::CreateMetrics(String& metrics, const String& MacAddress,
    ...
    for (int i = 0; i < _Protocol.InputRegisterCount; i++)
        metricsAddValue(_Protocol.InputRegisters[i].name,
                        getRegValue(&_Protocol.InputRegisters[i]), _Protocol.InputRegisters[i].resolution, metrics, labels);
    
    for (int i = 0; i < _Protocol.HoldingRegisterCount; i++)
        metricsAddValue(_Protocol.HoldingRegisters[i].name,
                        getRegValue(&_Protocol.HoldingRegisters[i]), _Protocol.HoldingRegisters[i].resolution, metrics,
                        labels);
...
#warning simulating the inverter
    ...
    metricsAddValue("HeapFragmentation", 100 - (100 * ESP.getMaxAllocHeap() / ESP.getFreeHeap()), 1, metrics, labels);
#else
    static uint32_t heap_min_free = ESP.getFreeHeap();
    heap_min_free = min(ESP.getFreeHeap(), heap_min_free);
    metricsAddValue("HeapMaxAlloc", ESP.getMaxFreeBlockSize(), 1, metrics, labels);
